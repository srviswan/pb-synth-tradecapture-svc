...
- Test with MS SQL Server (Testcontainers)
- Test partition state CRUD operations
- Test optimistic locking
- Test concurrent access scenarios
- Test archive operations
- Test filtered queries (non-archived records only)
...
- Test idempotency: duplicate trade ID returns cached result
- Test idempotency: duplicate with different payload is rejected
- Test idempotency: retry after failure is allowed
- Test idempotency: message redelivery is handled correctly
...
| Database | JDBC/JPA | H2 Embedded (for unit tests) | Testcontainers MS SQL Server | Test environment MS SQL Server |
